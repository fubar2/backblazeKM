...
for targ in dirs:
    ...
    for i,fn in enumerate(flist):
        ...
        for j,row in enumerate(td):
            ...
            tdict.setdefault(id,[dt,dt,'0',manu,'0']) # start,last,fail,0 hours if new
            rec = tdict.get(id)
            if dt < rec[1]:
               print('Earlier date %s record %s at row %d: %s' % (dt,rec[1],j,row))
            if dt > rec[1]:
               rec[1] = dt # update last obs
            if (fail=='1'):
               rec[2] = '1'
